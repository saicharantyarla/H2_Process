...
try:
	...
	if len(sys.argv) == 11:
		...
		ActivityLogger.InsertActivityLog(APID, CIServer, ActivityName, Des, "Initiated", LogAccountName, "No Error", "Connectivity check initiated", LogServer, LogDB, LogUser, LogPwd)
                HLStatus.UpdateHLStatus(APID,"Stepid86","In Progress","Oracle Installation Post Check is in Progress",LogServer,LogDB,LogUser,LogPwd)	
...
"""
		...
		if err:
			...
			HLStatus.UpdateHLStatus(APID,"Stepid86","Failed","Oracle Installation Post Check failed,Check the log for errors",LogServer,LogDB,LogUser,LogPwd)	
		else:
			if len(output1) == 0:
				log.info('ExitCode: 0')
				...
				HLStatus.UpdateHLStatus(APID,"Stepid86","Failed","Oracle Installation Post Check failed,Log files are validated ,Errors found. Check the logs for error",LogServer,LogDB,LogUser,LogPwd)	
	else:
		log.info('ExitCode: 1')
		log.info('ExitDesc: Missing Arguments')
		print "1"
		print "ExitDesc: Missing Arguments"
		HLStatus.UpdateHLStatus(APID,"Stepid86","Failed","Oracle Installation Post Check failed,Missing Arguments",LogServer,LogDB,LogUser,LogPwd)	
except socket.error:
	...
	print "ExitDesc: Connectivity to server %s failed" %address
        HLStatus.UpdateHLStatus(APID,"Stepid86","Failed","Oracle Installation Post Check failed,Check the Log for error",LogServer,LogDB,LogUser,LogPwd)	
except:
	#ActivityLogger.WriteActivityLog(APID, CIServer, ActivityName, Des, "Completed", LogAccountName, "Error","Check the Log for error", LogServer, LogDB, LogUser, LogPwd)
	print "1"
