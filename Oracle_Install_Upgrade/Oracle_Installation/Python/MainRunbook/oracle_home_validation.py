...
OFile = OFol + "\\" + 'oracle_home_validation.log'
# Variable Mapping #
ActivityName = "Validating ORACLE_HOME path"
Des = " Validating ORACLE_HOME path "
a = 0
s = socket.socket()
...
try:
	with open(OFile, 'w+') as of:
		...
		if len(sys.argv) == 11:
			...
			# Connectivity Test #
			HLStatus.UpdateHLStatus(APID,"Stepid96","In Progress","Oracle Home Validation Script Run is in Progress",LogServer,LogDB,LogUser,LogPwd)
			...
			ActivityLogger.WriteActivityLog( APID,CIServer,ActivityName,Des,"In Progress",LogAccountName, "No Error","Validating ORACLE_HOME path",LogServer,LogDB,LogUser,LogPwd)
			#print "hi"
			...
			if output1 == "No":
				...
				if output2 == "Yes":
					...
					print >> of, "[info]", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "The ORACLE_HOME path successfully validated"
					ActivityLogger.WriteActivityLog(APID, CIServer, ActivityName, Des, "Completed", LogAccountName,"No Error", "The ORACLE_HOME path successfully validated", LogServer, LogDB, LogUser, LogPwd)
					...
					print "0"
					print "ExitDesc: The ORACLE_HOME path successfully validated"
					...
					print >> of, "[info]", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "unable to create ORACLE_HOME path "
					ActivityLogger.WriteActivityLog(APID, CIServer, ActivityName, Des, "Completed", LogAccountName,"Error", "unable to create ORACLE_HOME path ", LogServer, LogDB, LogUser, LogPwd)
					...
					HLStatus.UpdateHLStatus(APID,"Stepid96","Failed","Oracle Home Validation Script Failed, unable to create ORACLE_HOME path",LogServer,LogDB,LogUser,LogPwd)
			else:
			...
			HLStatus.UpdateHLStatus(APID,"Stepid96","Failed","Oracle Home Validation Script Failed, Missing Arguments",LogServer,LogDB,LogUser,LogPwd)
except socket.error:
	...
	print "ExitDesc: Connectivity to server %s failed" %address
        HLStatus.UpdateHLStatus(APID,"Stepid96","Failed","Oracle Home Validation Script Failed, Connectivity to server failed",LogServer,LogDB,LogUser,LogPwd)
except:
	with open(OFile, 'a') as of1:
		ActivityLogger.WriteActivityLog(APID, CIServer, ActivityName, Des, "Completed", LogAccountName, "Error","Check the Log for error", LogServer, LogDB, LogUser, LogPwd)
	...
	HLStatus.UpdateHLStatus(APID,"Stepid96","Failed","Oracle Home Validation Script Failed, Some Exception Check Logs",LogServer,LogDB,LogUser,LogPwd)
