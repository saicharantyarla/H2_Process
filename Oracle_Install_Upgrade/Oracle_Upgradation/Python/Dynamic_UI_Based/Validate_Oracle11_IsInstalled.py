"""
	Name: Validate Oracle 11 is Installed
	Description: Executed from HP OO, Validate Oracle 11 in Installed
...
OFile = OFol + "\\" + 'Validate_Oracle_11_installed.log'
# Variable Mapping #
ActivityName = "Validate Oracle 11 is installed or not"
Des = " Validate Oracle 11 is installed or not"
a = 0
s = socket.socket()
...
try:
	with open(OFile, 'w+') as of:
		...
		if len(sys.argv) == 4:
			...
			print >> of, "[info]", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Validating oracle 11.2.0.4 is already Installed"
			...
			#ActivityLogger.WriteActivityLog( APID,CIServer,ActivityName,Des,"In Progress",LogAccountName, "No Error","Validating oracle 11 is installed",LogServer,LogDB,LogUser,LogPwd)
			#print "hi"

			command1="""cat /etc/oratab | grep "/11.2.0.4/" |grep -v '#' """
			#print command1
			...
			type(output1)
			if len(output1) == 0:
				print >>of,"[error]",datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") ,"ExitCode: 1"
				print >>of,"[error]",datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") ,"ExitDesc: Oracle 11.2.0.4 is not installed" 
				print "1"
				print "ExitDesc: Oracle 11.2.0.4 is not installed"
			else:
				output1 = output1.split("\n")
				makeitastring = ",".join(str(x) for x in output1)
				print >>of,"[error]",datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") ,"ExitCode: 0"
				print >>of,"[error]",datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") ,"ExitDesc:  Oracle 11.2.0.4 is  installed " 
				print "0"
				print "ExitDesc:  Oracle 11.2.0.4 is  installed " 
